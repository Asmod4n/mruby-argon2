def hashtest(version, t, m, p, pwd, salt, mcfref)
  hash, encoded = Argon2.hash(pwd, salt: salt, t_cost: t, m_cost: (1 << m), parallelism: p, version: version)

  if Argon2::VERSION_NUMBER == version
    assert_equal(encoded, mcfref)
  end

  Argon2.verify(encoded, pwd)
  Argon2.verify(mcfref, pwd)
end
